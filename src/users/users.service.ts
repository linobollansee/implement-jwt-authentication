...
// bcrypt für Passwort-Hashing importieren
...
export class UsersService {
  ...
  // findByEmail - retrieves a single user by email (for authentication)
  // findByEmail - ruft einen einzelnen Benutzer nach E-Mail ab (für Authentifizierung)
  async findByEmail(email: string): Promise<User | null> {
    // Search database for user with matching email
    // Datenbank nach Benutzer mit passender E-Mail durchsuchen
  ...
  async create(createUserDto: CreateUserDto): Promise<User> {
    ...
    // Passwort vor dem Speichern hashen (Sicherheits-Best-Practice)
    const hashedPassword = await bcrypt.hash(createUserDto.password, 10);

    // Create a new User entity instance with hashed password
    // Neue User-Entity-Instanz mit gehashtem Passwort erstellen
  ...
  async update(id: number, updateUserDto: UpdateUserDto): Promise<User> {
    ...
    // Passwort hashen, wenn es aktualisiert wird
